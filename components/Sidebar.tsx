...
const Sidebar: React.FC<SidebarProps> = ({ activeTab, setActiveTab }) => {
  ...
  return (
    ...
    >
      ...
      {/* Logo Header */}
      <div className={`flex flex-col items-center justify-center border-b border-white/10 bg-white/5 backdrop-blur-sm transition-all duration-300 ${isCollapsed ? 'p-4 py-6' : 'p-5'}`}>
        <div className={`${isCollapsed ? 'w-10 h-10 text-sm' : 'w-9 h-9 text-lg'} bg-gradient-to-tr from-pink-500 to-purple-600 rounded-lg flex items-center justify-center shadow-lg shadow-pink-500/20 transition-all duration-300`}>
          <span className="font-bold text-white">CGP</span>
        </div>
        <h1 className={`text-base font-bold tracking-wide text-white mt-2 transition-opacity duration-300 ${isCollapsed ? 'opacity-0 h-0 overflow-hidden mt-0' : 'opacity-100'}`}>
      ...
      <div className={`p-4 bg-black/20 backdrop-blur-md transition-all duration-300 ${isCollapsed ? 'flex justify-center' : ''}`}>
        <div className={`flex items-center gap-3 p-2 rounded-lg hover:bg-white/5 transition-colors cursor-pointer ${isCollapsed ? 'justify-center' : ''}`}>
          ...
          <div className={`overflow-hidden transition-all duration-300 ${isCollapsed ? 'w-0 opacity-0' : 'w-auto opacity-100'}`}>
            <p className="text-sm font-bold text-white truncate">Patrik Nazario</p>
            <p className="text-xs text-slate-400 truncate" title="patriknazario@ccgp.com.br">patriknazario@ccgp.com.br</p>
          </div>
